...
impl fmt::Display for Issue {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result { write!(f, "{} - {}", &self.key, &self.fields.get("summary").unwrap()) }
}
...
pub fn search_issues(config: Config, query: &str) -> Result<Vec<Issue>> {
    ...
    let resp = Client::new()
        ...
        .send().context("Unable to search JIRA for issues")?
        .json::<IssueSearchResponseBody>().context("Unable to decode JIRA response")?;
